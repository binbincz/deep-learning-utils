...
针对不同模型需要修改  model = SCRN() 中这个SCRN的对象
=================================================='''
...
def test_evaluate_sort(model_name, method_name, test_datasets, dataset_path, model_path, result_file_name, log_file_name, testsize, log_path="./log", is_need_process_image=True):
        ...
        for i,dataset in enumerate(test_datasets):
            startTime = time.time()
            logger.info("Model {} test NO:{} datasets {} begin test!".format(model_name,i,dataset))
...
if __name__ == '__main__':
    ...
    logger = log_set.setup_logger(log_file_name, log_file_path, 0)
    ...
    test_evaluate_sort(model_name, method_name, test_datasets, dataset_path,
                       model_utils.get_last_model(model_save_path), result_file_name,
